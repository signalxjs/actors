...
describe('bucketUpperBoundsUs', () => {
    it('answers null for a layout this build does not know', () => {
        // The same degradation `createMetricsAccumulator` gives a mismatched
        // digest: counts survive, the distribution is dropped — never a
        // distribution drawn on the wrong axis.
        expect(bucketUpperBoundsUs('ll-5-30')).toBeNull();
        expect(bucketUpperBoundsUs('')).toBeNull();
    });

    it('is strictly increasing and ends unbounded', () => {
        const bounds = bucketUpperBoundsUs(HISTOGRAM_LAYOUT)!;
        expect(bounds.length).toBe(384);
        for (let b = 1; b < bounds.length; b++) {
            expect(bounds[b]!).toBeGreaterThan(bounds[b - 1]!);
        }
        expect(bounds[bounds.length - 1]).toBe(Infinity);
    });

    it('brackets recorded values: every sample lands under its bucket bound', () => {
        // Cross-check the exported bounds against the recorder itself: for a
        // spread of values, the digest's occupied bucket must be the FIRST
        // whose upper bound exceeds the value — that is exactly the walk a
        // Prometheus renderer does to emit cumulative `le` counts.
        const bounds = bucketUpperBoundsUs(HISTOGRAM_LAYOUT)!;
        for (const us of [0, 1, 15, 16, 17, 255, 1_000, 65_536, 1_000_000, 2 ** 26, 2 ** 32]) {
            const h = new Histogram();
            h.record(us / 1000); // record() takes milliseconds
            const digest = h.digest();
            expect(digest.idx).toHaveLength(1);
            const bucket = digest.idx[0]!;
            expect(bounds[bucket]!).toBeGreaterThan(us);
            // The previous bucket's bound must NOT already cover the value —
            // otherwise the bound and the recorder disagree about the axis.
            if (bucket > 0) {
                expect(bounds[bucket - 1]!).toBeLessThanOrEqual(us);
            }
        }
    });
});
