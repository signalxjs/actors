 ...
 *
 * `clusterStats()` could always aggregate TOPOLOGY, because `SiloReport`
 * carried it. It could not aggregate behaviour, because the report carried
 * no metrics, no health and no grains — so a dashboard showed one silo's
 ...
 * p50/p90/p99 with no buckets, and the mean of two silos' p99s is not the
 ...
 *
 * **Coverage is published, not assumed.** A silo with no `metrics()`, or
 ...
 * cover all of it. `totals.metrics.silos` is the denominator that makes the
 * difference visible.
 */
import { afterEach, describe, expect, it } from 'vitest';
import { defineActor } from '@sigx/actors';
import { clusterStats } from '@sigx/actors/cluster';
import { createMetricsAccumulator, health, metrics, type MetricsPlugin } from '@sigx/actors/silo';
...
/** N silos, each with its own `metrics()`, each activating its own grains. */
 ...
 *
 * Strips the named keys from its `$sigx:silo#stats` response, which is what
 * a mid-rolling-deploy silo genuinely sends: still `v: 1`, just without the
...
describe('cluster ops: cluster-wide metrics', () => {
    it('sums calls, storage and churn across every silo', async () => {
        const { cluster } = await instrumented(3);
        running = cluster;
        // Different traffic per silo, so totals that actually read only ONE
        ...
        expect(totals!.silos).toBe(3);
        // 1 + 2 + 3 calls, each saving once and creating a grain.
    ...
    it('derives cluster latency from MERGED buckets, not averaged percentiles', async () => {
        ...
        running = cluster;
        for (let n = 0; n < 40; n++) await cluster.silos[0]!.actor(Counter, `fast-${n}`).increment(1);
        await cluster.silos[1]!.actor(Counter, 'slow').increment(1);

        const report = await clusterStats(cluster.placements[0]!);
        const merged = report.totals.metrics!.turnMs!;
        // Every sample from both silos is present — which is what makes
        // these the CLUSTER's percentiles rather than one silo's.
    ...
    it('publishes how many silos reported, so a partial picture cannot pass as whole', async () => {
        ...
        running = cluster;
        for (let i = 0; i < 3; i++) await cluster.silos[i]!.actor(Counter, `s${i}`).increment(1);

        const report = await clusterStats(cluster.placements[0]!);
        expect(report.totals.silos).toBe(3);
        expect(report.totals.metrics!.silos).toBe(2);
        // Not an error and not a partial fan-out: the third silo ANSWERED,
        // it simply has no metrics to give.
        expect(report.partial).toBe(false);
        expect(report.silos.some((s) => s.metrics === undefined)).toBe(true);
    });
    ...
    it('keeps a foreign bucket layout out of the distribution but not out of the counts', async () => {
        ...
        await cluster.silos[0]!.actor(Counter, 'a').increment(1);
        const report = await clusterStats(cluster.placements[0]!);
        const digest = report.silos[0]!.metrics!;
        ...
        // Dropping the silo entirely would be a worse lie than dropping its
        // percentiles.
    ...
    it('never lets a peer subtract from a cluster total', async () => {
        ...
        await cluster.silos[0]!.actor(Counter, 'a').increment(1);
        const report = await clusterStats(cluster.placements[0]!);
        const good = report.silos[0]!.metrics!;
    ...
    it('carries every silo’s health, not just the collector’s', async () => {
        const { cluster } = await instrumented(2);
        running = cluster;
        const report = await clusterStats(cluster.placements[0]!);
        // The health tally covers the fleet: a single-silo read cannot see
        ...
        expect(report.silos.every((s) => s.health !== undefined)).toBe(true);
    });
    ...
    it('reports readiness even where the health() ENDPOINT is not mounted', async () => {
        ...
        // registry, and `cluster()` always contributes one. So a silo with
        // no health endpoint still has a readiness answer to give, which is
        // the whole reason this rides `SiloReport` rather than an HTTP hop
        // to each peer.
        const { cluster } = await instrumented(2, (i) => i === 0);
        running = cluster;
        const report = await clusterStats(cluster.placements[0]!);
        expect(report.totals.health.unknown).toBe(0);
        expect(report.silos.every((s) => s.health?.checks.cluster !== undefined)).toBe(true);
    });
    ...
    it('counts a peer that sent no health at all as unknown, not as ready', async () => {
        ...
        const cluster = await createCluster(2, {
            ...
            policy: selfPolicy,
            plugins: () => [metrics()],
            wrapFetch: olderPeer('silo1', ['health'])
        });
        running = cluster;
        const report = await clusterStats(cluster.placements[0]!);
        expect(report.silos).toHaveLength(2);
...
describe('cluster ops: the detail flag', () => {
    it('leaves the grain list off unless detail asks for it', async () => {
        const { cluster } = await instrumented(2);
        running = cluster;
        for (let n = 0; n < 5; n++) await cluster.silos[0]!.actor(Counter, `g${n}`).increment(1);

        const cheap = await clusterStats(cluster.placements[0]!);
        // Grain KEYS are the one field on this wire that can be personal
        // data, and the walk is O(activations) on every silo at once.
        expect(cheap.silos.every((s) => s.activations === undefined)).toBe(true);
        // The digest, by contrast, always travels — it is what makes the
        // totals mean anything, and it is the cheap part.
        expect(cheap.silos.every((s) => s.metrics !== undefined)).toBe(true);

        const detailed = await clusterStats(cluster.placements[0]!, { detail: { activations: 3 } });
        const self = detailed.silos.find((s) => s.siloId === detailed.from)!;
        expect(self.activations!.length).toBeLessThanOrEqual(3);
        expect(self.activations![0]!.type).toBe('Counter');
    });

    it('asks only the named silos for the expensive parts', async () => {
        const { cluster } = await instrumented(3);
        running = cluster;
        for (let i = 0; i < 3; i++) await cluster.silos[i]!.actor(Counter, `g${i}`).increment(1);

        const target = cluster.placements[1]!.identity.siloId;
        const report = await clusterStats(cluster.placements[0]!, {
            detail: { activations: 5, silos: [target] }
        });
        // Drill-down is almost always ONE silo; asking all N for grain
        // lists at 1 Hz is the cost that actually matters.
        expect(report.silos.find((s) => s.siloId === target)!.activations).toBeDefined();
        expect(
            report.silos.filter((s) => s.siloId !== target).every((s) => s.activations === undefined)
        ).toBe(true);
    });
    ...
    it('clamps a hostile request at the RESPONDER', async () => {
        const { cluster } = await instrumented(1);
        running = cluster;
        for (let n = 0; n < 30; n++) await cluster.silos[0]!.actor(Counter, `g${n}`).increment(1);

        // The request arrives over a wire. HMAC proves WHO is asking; it
        // says nothing about whether walking a billion grains is a
    ...
    it('ignores nonsense in a request rather than failing the read', async () => {
        ...
        // The endpoint that explains a broken silo must not be the thing
...
describe('cluster ops: cost', () => {
    ...
    it('stays small enough to poll', async () => {
        ...
        // second per silo, and a breakdown that grows without anyone
