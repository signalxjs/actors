 ...
 * host cannot tell from `Counter#read` alone what is being asked of it —
...
describe('dispatchWatch in a cluster', () => {
    it('watches an actor placed on the LOCAL host', async () => {
        const h = await twoHosts();
        // prefer-local places it here on first touch.
        await h.hosts[0]!.actor(Counter, 'local').increment(1);
        ...
        await h.hosts[0]!.actor(Counter, 'local').increment(4);
        expect((await iterator.next()).value).toBe(5);
        await iterator.return?.();
    });

    it('watches an actor placed on ANOTHER host, and a mutation there arrives', async () => {
        const h = await twoHosts();
        // Place it on host 1, then watch from host 0 — the whole point.
        await h.hosts[1]!.actor(Counter, 'remote').increment(1);
        ...
        await h.hosts[1]!.actor(Counter, 'remote').increment(4);
        expect((await iterator.next()).value).toBe(5);
        await iterator.return?.();
    });
    ...
    it('carries the watch ARGUMENTS across the hop', async () => {
        ...
        const h = await twoHosts();
        await h.hosts[1]!.actor(Counter, 'args').increment(3);
    ...
    it('forwards throttleMs — it is per SUBSCRIPTION, not per actor', async () => {
        const h = await twoHosts();
        await h.hosts[1]!.actor(Counter, 'throttled').increment(1);
    ...
    it('DEDUPES on the owner: two watchers on one host cost one turn', async () => {
        // The dedupe that makes a live feed affordable has to survive the
        // hop, or fifty viewers on one edge host become fifty turns per
        // mutation on the owner — exactly what it exists to prevent.
        const h = await twoHosts();
        await h.hosts[1]!.actor(Counter, 'shared').increment(1);
        invocations = 0;

        const ref = { type: 'Counter', key: 'shared' } as const;
        const a = h.hosts[0]!.dispatchWatch!(ref, 'read', [], call())[Symbol.asyncIterator]();
        const b = h.hosts[0]!.dispatchWatch!(ref, 'read', [], call())[Symbol.asyncIterator]();
    ...
    it("a disconnect releases the OWNER's keep-alive", async () => {
        ...
        const h = await twoHosts();
        await h.hosts[1]!.actor(Counter, 'released').increment(1);
        ...
        const owned = (): boolean =>
            h.hosts[1]!.activations().some((a) => a.key === 'released' && a.keptAlive);
    ...
    it('the CLIENT channel pushes a remotely placed actor, over one connection', async () => {
        ...
        // mount on the host that served the page → placement → the owner on
        ...
        const h = await twoHosts();
        await h.hosts[1]!.actor(Counter, 'remote-live').increment(2);

        const transport = fetchTransport({
            endpoint: 'http://host0.test/_sigx/actor',
        ...
        try {
            ...
            await h.hosts[1]!.actor(Counter, 'remote-live').increment(3);
    ...
    it('refuses to watch a streams: method, on either side of the hop', async () => {
        ...
        // subscription.
        const h = await twoHosts();
        await h.hosts[1]!.actor(Counter, 'streamy').increment(1);

        const watching = h.hosts[0]!.dispatchWatch!(
