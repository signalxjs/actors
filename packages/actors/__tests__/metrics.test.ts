...
describe('metrics()', () => {
    ...
    it('splits queue wait from turn time', async () => {
        ...
        try {
            ...
            // Three 60ms turns issued at once against ONE actor. Turns
            // serialize, so the third waits ~120ms for a turn that
...
describe('metrics(): per-method breakdown', () => {
    ...
    it('records the queue/turn split per method', async () => {
        ...
        try {
            ...
            const snap = m.snapshot();
            // `slow` holds the activation; `noop` mostly waits for it. That is
