 ...
 * bearer token.
 *
 * It lives here rather than in either caller because a second copy of a
 * comparison like this is exactly how one of them quietly grows an early
 * `return false` — and that same reasoning is why it IS exported from
 * `@sigx/actors/host`: a separately-installed package mounting a
 * bearer-guarded endpoint (`@sigx/actors-otel`) must not carry its own copy.
 */
