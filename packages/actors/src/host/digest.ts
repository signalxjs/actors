...
export interface MetricsDigest {
     ...
     */
    windowMs: number;
    /** `oneWayFailures` is additive within `v: 1` — an older host's digest
     *  simply lacks the key and folds in as 0. */
    calls: { total: number; failed: number; streams: number; oneWayFailures?: number };
...
export function createMetricsAccumulator(): MetricsAccumulator {
    ...
    return {
        add(digest) {
            ...
            oneWayFailures += num(digest.calls?.oneWayFailures);
        ...
        totals() {
            ...
            return {
                calls: { total, failed, streams, oneWayFailures },
