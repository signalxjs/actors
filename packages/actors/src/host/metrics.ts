...
export interface ActorMetricsSnapshot {
    ...
    calls: {
        ...
        streams: number;
        /**
         * Turns accepted from a one-way dispatch that later threw — counted
         * on the host that ran the turn, never delivered to any caller.
         * Pre-acceptance failures count under `failed` instead. Optional so
         * a consumer (the CLI's HTTP source) can read an older host's
         * snapshot; this plugin always emits it.
         */
        oneWayFailures?: number;
    };
    /** End-to-end: enqueue to settle. Null when `histograms: false`. */
    latencyMs: HistogramSnapshot | null;
    /** Time waiting for its turn — high means the actor is HOT. */
    queueMs: HistogramSnapshot | null;
    /** Time holding the activation — high means the turn itself is SLOW. */
    turnMs: HistogramSnapshot | null;
...
export function metrics(options: MetricsOptions = {}): MetricsPlugin {
    ...
    const plugin: MetricsPlugin = {
        ...
        setup(registry: PluginRegistry): void {
            ...
            // Subscribed whenever collection is on — `disable()` still
            // detaches, so "off" still means absent. `oneWayFailures` is
            // counted here because the turn observer is the only place a
            // post-acceptance failure is still visible: the one-way caller's
            // promise resolved at enqueue, so the dispatch middleware above
            // never sees the throw. With `histograms: false` a minimal
            // observer attaches for the counter alone — which switches on
            // the activation's per-turn clock reads; the counter being
            // correct is judged worth that (noted in the CHANGELOG).
            subscribeTurns = () => {
                if (detachTurns || !host) return;
                const observer: ActorTurnObserver =
                    queue && turn
                        ? (ref, method, queuedMs, elapsedMs, turnFailed, call) => {
                              if (turnFailed && call?.oneWay === true) oneWayFailures++;
                        ...
                        : (_ref, _method, _queuedMs, _elapsedMs, turnFailed, call) => {
                              if (turnFailed && call?.oneWay === true) oneWayFailures++;
                          };
                detachTurns = host.observeTurns(observer);
            };
        ...
        snapshot(): ActorMetricsSnapshot {
            ...
            return {
                ...
                calls: { total: calls, failed, streams, oneWayFailures },
