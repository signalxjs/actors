...
/**
 * Upper bound in µs of each bucket in `layout`, or `null` for a layout this
 * build does not know — the caller degrades to counts-only, the same answer
 * `createMetricsAccumulator` gives a mismatched digest. Bucket `b` holds
 * values in `[valueOf(b), valueOf(b+1))`; the final (overflow) bucket is
 * unbounded, so its entry is `Infinity`.
 *
 * This is the export that lets a renderer (Prometheus `le` bounds, and the
 * like) map a wire digest's indices onto real values without core taking an
 * exporter dependency.
 */
export function bucketUpperBoundsUs(layout: string): number[] | null {
    if (layout !== HISTOGRAM_LAYOUT) return null;
    const bounds: number[] = [];
    for (let b = 0; b < BUCKETS - 1; b++) bounds.push(valueOf(b + 1));
