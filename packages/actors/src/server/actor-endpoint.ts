...
export interface ActorResolverOptions {
    ...
    maxHops?: number;
    /**
     * Most subscriptions one `$live` connection may open. Default 256; `0`
     * disables the cap. Must be a non-negative integer — anything else throws
     * rather than quietly disabling a security bound.
     *
     * `$live` fans out one watch per entry, all at once, and each can force a
     * distinct activation pinned for `idleAfterMs` — so without a cap a single
     * body-cap-sized request is the mount's largest amplification. Raise it for
     * a genuinely dense page; do not set `0` on a public listener.
     */
...
export async function handleActorRequest(
    ...
    const { host, onMiss, base, maxHops, maxLiveSubscriptions, streamPingMs, ...rest } = options;
    ...
    // Everything after the base IS the symbol (signalxjs/core#543), so the
    // routing hint has to come off the path before core reads it.
    ...
    const response = await handleServerFnRequest(routed, {
        ...
        // checks the path prefix itself (signalxjs/core#563) and defaults to
        // `/_sigx/fn`, so leaving it out 404s every actor call before
        // resolution — the failure that silently ate the whole wire surface
        // until signalxjs/core#563 made it say so. Same value the resolver
        // keys on, deliberately.
        base: actorBase,
...
function resolverFor(
    ...
    const key = [
        ...
        // Every option the resolver CLOSES OVER has to be in this key. Leave
        // one out and whichever mount constructed first silently decides it
        // for every other mount on the same host.
        options.maxLiveSubscriptions ?? DEFAULT_MAX_LIVE_SUBSCRIPTIONS
    ].join('|');
...
export function createActorResolver(
    ...
    return (wire: string) => {
        ...
        if (symbol === LIVE_SYMBOL) {
            const live = synthesizeLive(host, options.maxLiveSubscriptions);
...
function synthesize(
    ...
    const prepare = async (
        ...
        return {
            ...
            call: {
                ...
                ...(rq.request.headers.get(ACTOR_ONEWAY_HEADER) === '1'
     ...
     * same turn. A GET is the same call arriving another way.
     */
