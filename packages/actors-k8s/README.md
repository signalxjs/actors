...
Silo liveness and peer discovery ride `coordination.k8s.io` **Leases** — the
...
re-read on rotation. Call `k8sMembership()` once per silo.
...
| `leasePrefix` | `sigx` | Lease names are `{leasePrefix}-{siloId}` |
...
Each silo owns one Lease; nobody else ever writes it:
...
```json
{
    ...
    "metadata": {
        ...
        "annotations": { "sigx.dev/descriptor": "{\"siloId\":\"s.k3f9a2\",…}" }
    },
    "spec": {
...
- **Discovery** — every silo runs the standard list-then-watch loop over
...
- **Self-fencing** — renewal failures past `ttlMs` fire `onSelfSuspect`
  once, exactly like the Redis provider: the silo stops claiming actors and
...
- **Graceful exit** — `setStatus('leaving')` patches the descriptor
  ...
  Lease. A crashed silo simply stops renewing and ages out.
...
`renewTime` is written by each silo's own clock and compared against the
...
Every renewal is a watch event delivered to every silo: n silos beating
every 5 s ≈ n²/5 events per second cluster-wide. At tens of silos this is
trivial (30 silos ≈ 180 tiny JSON lines/s, and none of them touch the
view). For hundreds of silos, raise `heartbeatMs`/`ttlMs` — the volume
falls quadratically.

## RBAC

The ServiceAccount needs Lease access in the silo namespace, nothing else:
