...
- `examples/dashboard` — `@sigx/actors-dashboard` against a REAL cluster
  (`pnpm --filter counter-example cluster:serve`, three hosts on 5391–5393),
  on **5490**. Its subject is the ~60-line `src/ops-proxy.ts`: `ops()` sets no
  CORS headers and refuses to construct without a bearer secret outside
  `__DEV__`, so a browser dashboard MUST go through a same-origin route of the
  consuming app that attaches the token server-side — the secret never reaches
  the client, and `src/main.tsx` is nine lines with no secret and no host
  address in it. The proxy is one handler mounted by both `vite.config.ts`
  ...
  `cluster:serve` kills the OWNER of the `cart` actor — placement decides who
  that is, so the casualty differs run to run and any hardcoded port is a
  one-in-three chance of aiming at the corpse (#256), which is why the proxy
...
- `perf/aks` → `sigx-perf-aks` — the production-shaped Kubernetes
  ...
  check `helm lint` cannot be. `__tests__/chart-equivalence.test.ts`
  (#59) is the other guard: the two charts were hand-copied from one
  another, so it renders both and asserts the shared hardening lessons
  (probes, PDB, node spread, resources, drain budget) still agree —
  outcomes, never bytes. It runs wherever `helm` is on PATH (the hosted
  runners preinstall one, so the main CI matrix covers it; `charts.yml`
  runs it first, before the cluster) and skips with the reason as its
  skip note otherwise. Not published.
