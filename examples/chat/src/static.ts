 ...
 * the socket directly — can send `/../package.json`. Handing that straight
 * to `join`/`resolve` collapses the `..` for you and reads the file, so the
 ...
 *
 * The return value is absolute. `root` is normalized first, so a trailing
 * separator does not make the `root + sep` check below reject every file
 * genuinely inside it.
 */
export function resolveStatic(root: string, target: string): string | null {
    ...
    // `=== base` covers the root itself; the `+ sep` stops `/assets-other`
    // passing a naive prefix check against `/assets`.
