...
/**
 * The workflow engine under host loss (#297) — three in-process hosts on a
 * `memoryClusterHub`, one shared `memoryStorage` as the cluster's database,
 * the `examples/counter/job-demo.mjs` recipe.
 *
 ...
 *
 * `WF_DEACTIVATE_ON_SLEEP=0` in this file, deliberately: a run that
 * deactivates the moment it sleeps has no owner to kill. The unit suite
 * covers the deactivating shape.
 */
...
describe('WorkflowRun across hosts', { timeout: 20_000 }, () => {
    it('places runs across the cluster', async () => {
        const version = await seed({ delayMs: 20 });
        const ids = Array.from({ length: 12 }, () => fresh('spread'));
        await Promise.all(ids.map((id) => run(id).start({ workflow: 'order', version, template: 'order', tag: 'spread' })));
    ...
    it('the stats aggregator re-places after its owner dies and keeps counting', async () => {
        ...
        await Promise.all(ids.map((id) => run(id).start({ workflow: 'order', version, template: 'order', tag: 'stats2' })));
