...
import { benchCall, createBenchHost, refsFor, warmActivations } from '../host-fixture.ts';
import type { Metric, RunContext, Scenario } from '../types.ts';

/**
 * Activate a never-seen key, then deactivate it. Measures the pair, since
 * measuring activation alone would leave the host holding every activation
        ...
        try {
            ...
            const outcome = await closedLoop({
                call: async (i) => {
                    ...
                    await fixture.host.deactivate(ref);
                },
...
const sweeperScan: Scenario = {
    ...
    async run(ctx: RunContext): Promise<Metric[]> {
        ...
        for (const size of sizes) {
            const fixture = await createBenchHost({ actors: [Tiny] });
            try {
                await warmActivations(fixture.host, refsFor(Tiny.type, size));
...
const reminderTick: Scenario = {
    ...
    async run(ctx: RunContext): Promise<Metric[]> {
        ...
        const fixture = await createBenchHost({ actors: [Reminded] });
        try {
            // onActivate registers one durable reminder per activation.
            await warmActivations(fixture.host, refsFor(Reminded.type, count), 'current');
            const started = performance.now();
            const passes = 10;
